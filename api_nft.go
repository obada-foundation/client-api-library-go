...
*/
func (a *NFTApiService) UpdateMetadata(ctx context.Context, key string) ApiUpdateMetadataRequest {
	return ApiUpdateMetadataRequest{
		ApiService: a,
		ctx: ctx,
		key: key,
...
// Execute executes the request
func (a *NFTApiService) UpdateMetadataExecute(r ApiUpdateMetadataRequest) (*http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "NFTApiService.UpdateMetadata")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/nft/{key}/metadata"
	localVarPath = strings.Replace(localVarPath, "{"+"key"+"}", url.PathEscape(parameterToString(r.key, "")), -1)
